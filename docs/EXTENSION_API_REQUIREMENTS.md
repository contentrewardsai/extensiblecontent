...
- **Cloud templates (sync with Whop web app):**
  - `GET/POST /api/extension/shotstack-templates`, `GET/PATCH/DELETE /api/extension/shotstack-templates/[id]` — `edit` is ShotStack timeline JSON; `default_env`: `stage` \| `v1`. Used for all template types. **Extension:** on save / panel load, POST or GET to merge with local templates.

**ExtensibleContentExtension repo:** On template save, call `POST` or `PATCH` the route above with `Authorization: Bearer <access_token>`. On side panel load, `GET` the list and merge with local files (merge strategy: by stable `id` from server, or name + `updated_at`). The Whop web app at `/experiences/[experienceId]/shotstack` reads the same table.
