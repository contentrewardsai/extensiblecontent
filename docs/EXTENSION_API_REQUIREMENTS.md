...
- GET /api/extension/sidebars, POST /api/extension/sidebars/register, POST /api/extension/sidebars/disconnect, POST /api/extension/sidebars/heartbeat (single: `sidebar_id` or `window_id`; MCP batch: `backend_ids: string[]` of Supabase UUIDs, max 64, deduped), GET/PATCH/POST /api/extension/sidebars/[id] (POST aliases PATCH for some relays)
