...
- GET /api/extension/sidebars, POST /api/extension/sidebars/register (`window_id` ≤512 chars, `sidebar_name` ≤256), POST /api/extension/sidebars/disconnect (exactly one of `sidebar_id` or `window_id`), POST /api/extension/sidebars/heartbeat (single: exactly one of `sidebar_id` or `window_id`; batch: `backend_ids: string[]` only — Supabase UUIDs, max 64, deduped; response includes `updated`, `requested`, `skipped`, `sidebars`), GET/PATCH/POST /api/extension/sidebars/[id] (POST aliases PATCH for some relays; non-empty `sidebar_name` ≤256)
