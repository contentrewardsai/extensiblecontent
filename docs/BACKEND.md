...
| Method | Path | Purpose |
|--------|------|--------|
| GET, HEAD | `/api/extension/sidebars` | List rows for the user (`HEAD` runs the same auth, query validation, and DB filter; empty body, same cache headers). Each item includes `connected` unless `omit_connected=1`/`true`/`yes`. Optional: `since` (ISO 8601) — `last_seen >= since`; `limit` (1–200). Response `Cache-Control: private, no-store`, `Vary: Authorization`. |
...
**ExtensibleContentExtension:** `SidebarsApi.listSidebars({ since, limit })` appends query params to `GET /api/sidebars` on the MCP server; the server **proxies** `since` / `limit` to the host via the relay when connected, otherwise falls back to the local in-memory list. Full patch: [docs/patches/extensible-content-extension-sidebar-heartbeat.patch](patches/extensible-content-extension-sidebar-heartbeat.patch). From a clone of that repo: `git apply /path/to/this-repo/docs/patches/extensible-content-extension-sidebar-heartbeat.patch` (or cherry-pick if you maintain a fork).
