...
| Method | Path | Purpose |
|--------|------|--------|
| GET | `/api/extension/sidebars` | List rows for the user; each item includes `connected` (true if `last_seen` within ~1 hour) unless `omit_connected=1`/`true`/`yes`. Optional: `since` (ISO 8601) — `last_seen >= since`; `limit` (1–200). Response `Cache-Control: private, no-store`, `Vary: Authorization`. |
