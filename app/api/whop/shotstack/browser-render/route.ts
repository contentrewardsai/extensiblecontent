...
export async function POST(request: NextRequest) {
	...
	if (!experienceId || !template_id) {
		return Response.json({ error: "experienceId and template_id are required" }, { status: 400 });
	}
	...
	// The client may fall back to the raw WebM if FFmpeg conversion fails. Trust the blob's
	// MIME to choose the extension so players don't choke on a .mp4 that is actually WebM.
