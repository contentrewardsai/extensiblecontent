...
export default async function MediaBillingPage({
	params,
	searchParams,
}: {
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				<h2 className="text-6 font-bold text-gray-12">Render billing</h2>
				<p className="text-3 text-gray-10 mt-1 max-w-2xl">
					...
					offset by an <em>Expired (rolled off)</em> entry that links back to the original grant.
				</p>
				<p className="text-2 text-gray-10 mt-2">
					<Link href={`/experiences/${experienceId}/media`} className="underline text-gray-12">
						← Back to Media
					</Link>
				</p>
			</div>
			...
			{(pageNum > 1 || hasNext) ? (
				<nav className="flex items-center justify-between text-3 text-gray-11">
					{pageNum > 1 ? (
						<Link
							href={`/experiences/${experienceId}/media/billing?page=${pageNum - 1}`}
							className="underline text-gray-12"
						>
							← Newer
						</Link>
